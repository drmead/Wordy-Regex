...
The intention is that it will support the humane notation when there are modules
to support it - in the meantime it only supports the wordy notation.
...
sub hre_form {
    ...
    my $cb_m_mode_terse   = $cgi->checkbox('m_mode_terse'    , $m_mode_terse,'checked','/m mult-line   ');
...
<form name ="hre" action="hre_resp" method="post">
    ...
    <br>
    <h2>Terse</h2>
    <h2>$cb_m_mode_terse $cb_s_mode_terse $cb_i_mode_terse</h2>
    ...
    <textarea name="hre" rows ="15" cols="80">$hre_box_content</textarea>
...
#   <p>after</p>
  ...
  sub create_hre_form {
    ...
    # ( so new it has yet to be written) humane-to-terse and terse-to-humane
     ...
     eval {
        if (lc $chosen =~ /to terse/i) {
            ...
            my $modes = ($x_mode_terse ? 'x' : '')
                      ...
                      . ($s_mode_terse ? 's' : '')
                      . ($i_mode_terse ? 'i' : '');
            $new_hre = tre_to_wre($tre_in, $modes);
            ...
            my @results = $text_in =~ $hre_obj;
            ## my @results = $text_in =~ m/$hre_obj->{terse}/x;
            ### my @results = $text_in =~ m/$hre_obj/x;
            my $named_results = '';
            foreach my $key (sort keys %+) {
                $named_results .= "$key: $+{$key}\n";
            }

            $new_tre = $hre_obj->{terse};
            if (scalar @results == 0 && $named_results eq '') {
                # No matches
            } else {
                # Matched
                $new_results = $named_results
                             . join("\r\n", @results)
                             . "\r\nStart Positions:\r\n"
                             . join("\r\n", @-)
                             . "\r\nEnd Positions:\r\n"
